...
export const aiAssistantRepository = {
  async getStoryContext(userId: string, storyId?: string) {
      ...
      if (author) {
        ...
        if (story) {
          storyContext = {
            ...
            status: story.status,
            slug: story.slug,
            tags: story.StoryTag.map((st: any) => st.Tag),
            themes: story.StoryTheme.map((st: any) => st.Theme),
          };
        }
      }
    }
  ...
  async getSuggestions(userId: string, storyId: string, contentType: string) {
    const userProfile = await prisma.userProfile.findUnique({
      where: { id: userId },
      select: { name: true },
    });

    if (!userProfile?.name) return { suggestions: [] };
  ...
  async getSeoSuggestions(userId: string, storyId: string) {
    const userProfile = await prisma.userProfile.findUnique({
      where: { id: userId },
      select: { name: true },
    });
    ...
    const existingKeywords =
      story.seoKeywords
        ...
        .map((k) => k.trim())
        .filter(Boolean) ?? [];
    const tagNames = story.StoryTag.map((st: any) => st.Tag.name);

    // Compute a basic SEO score
    let score = 0;
    const suggestions: string[] = [];

    if (story.seoTitle && story.seoTitle.length > 10) score += 25;
    else
      suggestions.push("Add a descriptive SEO title (minimum 10 characters)");

    if (story.seoDescription && story.seoDescription.length > 50) score += 25;
    else
      suggestions.push(
        "Write an SEO meta description (50-160 characters recommended)",
      );

    if (story.excerpt && story.excerpt.length > 80) score += 20;
    else
      suggestions.push(
        "Add a compelling story excerpt for better search visibility",
      );

    if (existingKeywords.length >= 3) score += 15;
    else suggestions.push("Add at least 3 SEO keywords");

    if (tagNames.length >= 2) score += 15;
    else suggestions.push("Add more tags to improve content categorization");
  ...
  async generateTitleSuggestions(userId: string, storyId: string) {
    const userProfile = await prisma.userProfile.findUnique({
      where: { id: userId },
      select: { name: true },
    });

    if (!userProfile?.name) return { titles: [] };
  ...
  async generateOutline(userId: string, storyId: string) {
    const userProfile = await prisma.userProfile.findUnique({
      where: { id: userId },
      select: { name: true },
    });

    if (!userProfile?.name) return { outline: [] };
