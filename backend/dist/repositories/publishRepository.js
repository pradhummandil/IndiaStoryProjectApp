...
exports.publishRepository = {
    ...
    async validateForPublish(storyId, userId) {
        const story = await prismaClient_1.prisma.story.findFirst({
            where: { id: storyId, deleted: false },
            include: {
                StoryImage: { take: 1 },
        ...
        if (!story)
            return { valid: false, errors: ["Story not found"] };
        const errors = [];
        const warnings = [];
    ...
    async publishStory(storyId, userId, scheduledAt) {
        const story = await prismaClient_1.prisma.story.findFirst({
            where: { id: storyId, deleted: false },
        });
        if (!story)
        ...
        await prismaClient_1.prisma.storyRevision.create({
            data: {
                id: uuidv4(),
                storyId: story.id,
                title: story.title,
